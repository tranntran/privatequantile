...
originalKNG = function(data, total_eps, tau, nbatch = 10000, scale = 1e-4, 
  ...
  for (i in 1:length(tau)){
    ...
    temp = KNG(init = rep(0, ncol(X)), ep = ep, tau = tau[i], sumX = sumX, X = X, 
               Y = Y, nbatch = nbatch, scale = scale/R, nonneg = nonneg,
  ...
  for (k in 1:length(vars)){
    ...
    if (syn_var == "x1"){
      ...
      print(ggplot(plotdata, aes(x=value, fill = L1)) + facet_wrap(~L1, ncol = 4) +
              ...
              labs(fill='Methods')  + coord_cartesian(xlim=c(-200, 100)) +
      ...
      temp = sandwichKNG(data = data, total_eps = ep, median_eps = 0.8, main_tau_eps = 0.8,
                         ...
                         method = "fixed", nonneg = TRUE, lower_accept = 0.2, 
                         upper_accept = 0.6, update_after = 10, adjust_scale_by = 2, 
                         formula = mod)
      ...
      temp = sandwichKNG(data = data, total_eps = ep, median_eps = 0.9, main_tau_eps = 0.9,
                         ...
                         method = "varying_newdata", nonneg = TRUE, lower_accept = 0.2, 
                         upper_accept = 0.6, update_after = 10, adjust_scale_by = 2, 
      ...
      print(ggplot(plotdata, aes(x=value, fill = L1)) + facet_wrap(~L1, ncol = 4) +
              ...
              scale_color_brewer(palette="Dark2") + theme_minimal() + xlim(c(-500, 1000))+
