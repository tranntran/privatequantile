...
stepwiseKNG = function(data, total_eps, median_eps = NULL, tau, scale = 1e-4,
  ...
  out = KNG(init = rep(6, m+1), ep = total_eps*median_eps, tau = 0.5, sumX = sumX, X = X, 
...
originalKNG = function(data, total_eps, tau, nbatch = 10000, scale = 1e-4, 
  ...
  for (i in 1:length(tau)){
    ...
    temp = KNG(init = rep(6, m+1), ep = ep, tau = tau[i], sumX = sumX, X = X, 
  ...
  for (k in 1:length(vars)){
    ...
    if (syn_var == "x1"){
      ...
      temp = originalKNG(data = data, total_eps = ep, tau = tau, nbatch = runs,
                         scale = 0.03, lower_accept = 0, upper_accept = 1,
                         nonneg = TRUE, formula = fml, update_after = 10, 
                         adjust_scale_by = 2)
      all_beta[[1]] = temp[[1]]
      accept_x1[[1]] = temp[[3]]
      scale_x1[[1]] = temp[[2]]
      ...
      temp = originalKNG(data = data, total_eps = ep, tau = tau, nbatch = 10000,
                         ...
                         nonneg = TRUE, formula = mod, update_after = 10, 
                         adjust_scale_by = 2)
      temp
      all_beta[[1]] = temp[[1]]
      if (syn_var == "x2") {
      ...
      temp = stepwiseKNG(data = data, total_eps = ep, median_eps = 0.25, 
                         tau = tau, scale = 0.00005, nbatch = 10000, method = "currentdata", 
                         nonneg = TRUE, lower_accept = 0, upper_accept = 1, 
                         update_after = 10, adjust_scale_by = 2, formula = mod)
      all_beta[[3]] = temp[[1]]
      if (syn_var == "x2") {
      ...
      temp = sandwichKNG(data = data, total_eps = ep, median_eps = 0.25, main_tau_eps = 0.8,
                         tau = tau, main_tau = main_tau, scale = 0.05, nbatch = runs, 
                         method = "koenker", nonneg = TRUE, lower_accept = 0, upper_accept = 1, 
                         update_after = 10, adjust_scale_by = 2, formula = mod)
      ...
      temp = sandwichKNG(data = data, total_eps = ep, median_eps = 0.25, main_tau_eps = 0.5,
                         tau = tau, main_tau = main_tau, scale = 1e-3, nbatch = runs, 
                         method = "currentdata", nonneg = TRUE, lower_accept = 0, 
...
mod = glm(inds ~ ., data = ut.data[[4]], family = binomial(link = "logit"))
preds = predict(mod, type = "response")
score = sum((preds-0.5)^2)/nrow(data)
score
