...
library(data.table)
rm(list = ls())
filename = "stepwise_fixed_sd_3_e0.1"
...
stepwise_fixed_int$Eps = allocate_eps
...
ggplot(stepwise_fixed_int, aes(x = Quantile, log(Value))) +
...
stepwise_fixed_slope$Eps = allocate_eps
...
ggplot(stepwise_fixed_slope, aes(x = Quantile, log(Value))) +
  geom_line(aes(group = Eps, color = Eps, linetype = Eps),  size = 1) +
  scale_x_continuous(breaks= tau) +
...
stepwise_fixed_l2$Eps = allocate_eps
...
png(paste("plot/", filename, "_distance_meanl2.png", sep = ""), 
    width = 600, height = 400)
ggplot(stepwise_fixed_l2, aes(x = Quantile, log(Value))) +
  geom_line(aes(group = Eps, color = Eps, linetype = Eps),  size = 1) +
  scale_x_continuous(breaks= tau) +
