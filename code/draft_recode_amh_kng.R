...
amh = function(logf, init = c(1, 1)/R, nbatch = 100000, scale, nonneg = FALSE){
  ...
  for (i in 2:nbatch){
    ...
    if ((log(u) < logf(c(propose_val)) - logf(batch[i-1, ])) & check){
      ...
      j = j +1
      ct = ct + 1
      cor_batch = rbind(cor_batch, propose_val)
      #print(ct)
    } else {
      batch[i, ] = batch[i-1, ]
    }
    if (ct == 10){
      # print(cor_batch)
      # if (length(cor_batch) > 100) {
      #   sigma = cor(na.omit(tail(cor_batch, 100)))
      # } else {
      #   sigma = cor(na.omit(cor_batch))
      # }
      # j = j + 1
      # avg_sigma[[j]] = sigma
      # sigma = Reduce("+", avg_sigma) / length(avg_sigma)
      #sigma = ifelse(length(cor_batch) > 100, cor(na.omit(tail(cor_batch, 100))), cor(na.omit(cor_batch)))
      if (j > 100) {
        sigma = cor(na.omit(batch[(j-100):j, ])) + diag(dim)*0.00001
      } else {
        sigma =  cor(na.omit(batch)) + diag(dim)*0.00001
      }
      print(sigma)
      #sigma = cor(na.omit(batch)) #+ diag(dim)*0.00001
      # sigma[1, 1]= sigma[1, 1]#/2#*5
      # sigma[1, 2] = sigma[1, 2]#/sqrt(5) #7
      # sigma[2, 1] = sigma[2, 1]#/sqrt(5)
      # sigma[2, 2]= sigma[2, 2]#*10#/5
      #sigma = sigma*0.1
      temp_acc = mean(accept[1:i])
      if (temp_acc < 0.2){ #prev 0.1 and 0.2
        scale_sigma = scale_sigma/2#*0.1
      } else if (temp_acc > 0.7) {
        print(temp_acc)
