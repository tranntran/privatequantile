...
stepwise_data_int$Eps = allocate_eps
...
ggplot(stepwise_data_int, aes(x = Quantile, log(Value))) +
...
stepwise_data_slope$Eps = allocate_eps
...
ggplot(stepwise_data_slope, aes(x = Quantile, log(Value))) +
  geom_line(aes(group = Eps, color = Eps, linetype = Eps),  size = 1) +
  scale_x_continuous(breaks= tau) +
...
stepwise_data_l2$Eps = allocate_eps
...
png(paste("plot/", filename, "_distance_meanl2.png", sep = ""), 
    width = 600, height = 400)
ggplot(stepwise_data_l2, aes(x = Quantile, log(Value))) +
  geom_line(aes(group = Eps, color = Eps, linetype = Eps),  size = 1) +
  scale_x_continuous(breaks= tau) +
