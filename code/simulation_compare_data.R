# This file was last updated April 19

# This code does not need any input to run.
# The output of this code is used in compare_pMSE_utility.R
...
# this function is used to get synthetic values from a vector of randomly 
# selected quantiles
...
compare_methods = function(data1, holdout_dat = holdout_dat,
  ...
  for (k in 1:length(vars)){
    ...
    if (syn_var == "x1"){
      ...
      temp = sandwichKNG(data = data, total_eps = ep, median_eps = 0.25, Cx = 1,
                         main_tau_eps = 0.6, tau = tau, main_tau = main_tau, 
                         scale = 0.1, sw_scale = 0.03, nbatch = runs, method = "fixed", 
                         lb = 0, ub = 1000, formula = fml)
      all_beta[[4]] = temp[[1]]
      ...
      temp = sandwichKNG(data = data, total_eps = ep, median_eps = 0.25, Cx = 1,
                           main_tau_eps = 0.6, tau = tau, main_tau = main_tau, 
                           scale = 0.1, sw_scale = 0.03, nbatch = runs, method = "varying", 
                           lb = 0, ub = 1000, formula = fml)
      all_beta[[5]] = temp[[1]]
...
save(oper, file = "../output/simulation_data_eps1.Rdata")