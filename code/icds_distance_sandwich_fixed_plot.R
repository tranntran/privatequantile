...
library(data.table)
rm(list = ls())
filename = "sandwich_fixed_sd_2_e0.1"
...
sandwich_fixed_int$Eps = allocate_eps
...
ggplot(sandwich_fixed_int, aes(x = Quantile, log(Value))) +
...
sandwich_fixed_slope$Eps = allocate_eps
...
ggplot(sandwich_fixed_slope, aes(x = Quantile, log(Value))) +
  geom_line(aes(group = Eps, color = Eps, linetype = Eps),  size = 1) +
  scale_x_continuous(breaks= tau) +
...
png(paste("plot/", filename, "_distance_meanl2.png", sep = ""), 
    width = 600, height = 400)
ggplot(sandwich_fixed_l2, aes(x = Quantile, log(Value))) +
  geom_line(aes(group = Eps, color = Eps, linetype = Eps),  size = 1) +
  scale_x_continuous(breaks= tau) +
