...
for (j in 1:reps){
# compare_methods = function(data, tau = c(seq(0.05, 0.95, 0.05), 0.99), 
#                            main_tau = c(0.05, 0.25, 0.5, 0.75, 0.95, 0.99), )

  print(j)
  data1 = syn_data(param = c(l, a0, b0, l), nVar = 2, nObs = n)
  vars = c("x1", "x2")
  colnames(data1) = vars
  ep = toteps/2
  ...
  check = 0
  while(check == 0){
    pmse = mcmc::metrop(densitySamp, initial = c(1e-4, a0, b0, 1e-4), nbatch = 100, scale = 0.4, 
                        ep = toteps, origData = data1, nSyn = 25, nVar = 2, nObs = n, lambda = 100000)
    check = pmse$accept
  }
  pmse_res = tail(pmse$batch, 1)
  ans_pmse = rbind(ans_pmse, pmse_res) 
  pmse_res[, c(1, 4)] = abs(pmse_res[, c(1, 4)])
  syn_pmse = syn_data(pmse_res, 2, n)
  ...
  for (k in 1:length(vars)){
    ...
    if (syn_var == "x1"){
      ...
      filename = paste(paste0('seed', t), paste0('rep', j), paste0('e', ep*2), syn_var, sep = '_')
      png(paste("./plot/simulations/pMSE/eps1/simulation_", filename, ".png", sep = ""), 
      ...
      temp = sandwichKNG(data = data, total_eps = ep, median_eps = 0.6, main_tau_eps = 0.7,
                         tau = tau, main_tau = main_tau, scale = 0.1, sw_scale = 0.03,
...
# library(doParallel)
# oper <- foreach(i=1:2, .combine=rbind, .multicombine=TRUE,
